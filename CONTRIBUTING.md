...
9. Bump the OUI version number in Optimizely's [`package.json`](https://github.com/optimizely/optimizely/blob/devel/src/www/frontend/package.json).
   ...
   - You'll also need to bump the OUI version of each SCS in the `src/www/frontend/self_contained_systems` directory (e.g. `src/www/frontend/self_contained_systems/datafile_access_tokens/frontend/package.json`)
     - Example (`butterbar_manager` SCS): within the Optimizely SCS folder at `src/www/frontend/self_contained_systems/butterbar_manager/frontend` run `yarn upgrade optimizely-oui@99.xx.xx` (note to include the major version number e.g. `99.`, leaving the minor and patch numbers as `xx.xx`) which updates `yarn.lock` and `package.json`
